...
const fetchWeather = async () => {
  const city = document.querySelector('#city').value.charAt(0).toUpperCase();
  const country = document.querySelector('#country').value.charAt(0).toUpperCase();

  try {
    const API_KEY = 'cf701ba1fa0b6880bd6c1a3d23e41be5';
    const proxy = 'https://cors-anywhere.herokuapp.com/';
    const api_call = await fetch(`${proxy}https://api.openweathermap.org/data/2.5/weather?q=${city},${country}&appid=${API_KEY}&units=metric`);
